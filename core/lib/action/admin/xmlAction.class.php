...
class xmlAction extends baseAction{
	...
	// 资源站1(jisucms 3.x api json)
    public function xml_jisu($array_url){
		//组合资源库URL地址并获取XML资源
		$http_url = str_replace('@', '-', admin_js_url_repalce($array_url['xmlurl']).'index.php?s=plus-api-json-action-'.$array_url['action'].'-vodids-'.$array_url['vodids'].'-cid-'.$array_url['cid'].'-play-'.$array_url['play'].'-inputer-'.$array_url['inputer'].'-wd-'.urlencode($array_url['wd']).'-h-'.$array_url['h'].'-p-'.$array_url['page']);

        $json = js_file_get_contents($http_url);
        ...
        print_r($json);
			...
			foreach($json->data as $key=>$value){
				...
				$array_vod[$key]['vod_cid'] = intval(js_bind_id($array_url['fid'].'_'.$value->vod_cid));
	...
	//资源站为A类型
    public function xml_caijia($array_url){die('bb');
		//组合资源库URL地址并获取XML资源
		$array_tpl['httpurl'] = '&wd='.urldecode($array_url['wd']).'&t='.$array_url['cid'].'&h='.$array_url['h'].'&ids='.$array_url['vodids'].'&pg='.$array_url['page'];
		if($array_url['action']){
			$array_tpl['httpurl'] = str_replace('?ac=list','?ac=videolist',admin_js_url_repalce($array_url['xmlurl'])).$array_tpl['httpurl'];
		}else{
			$array_tpl['httpurl'] = admin_js_url_repalce($array_url['xmlurl']).$array_tpl['httpurl'];
		}
		$xml = js_file_get_contents($array_tpl['httpurl']);
		if ($xml) {
			//组合分页信息
			...
			foreach($vod_array[1] as $key=>$value){
				...
				$listvod[$key]['vod_cid'] = intval(js_bind_id($array_url['fid'].'_'.$vod_array[3][$key]));
				...
				$listvod[$key]['vod_reurl'] = admin_js_url_repalce($array_url['reurl']).$listvod[$key]['vod_id'];
				if($array_url['action']){
	...
	//将资源站A类型XML方式获取到的播放地址转化为极速的播放地址格式
