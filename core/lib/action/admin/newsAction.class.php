...
class newsAction extends baseAction{
    ...
    public function show(){
		...
		foreach($list as $key=>$val){
			...
			$list[$key]['news_url'] = js_data_url('news',$list[$key]['news_id'],$list[$key]['news_cid'],$list[$key]['news_name'],1,$list[$key]['news_jumpurl']);
    ...
    public function _before_insert(){
		...
		if(empty($_POST["news_keywords"]) && C('rand_tag')){
			$_POST["news_keywords"] = js_tag_auto($_POST["news_name"],$_POST["news_content"]);
		}
	}	
	// 新增新闻保存到数据库
    ...
    public function delfile($id){
		...
		@unlink(js_img_url($arr['news_pic']));
		if(C('url_html')>0){
			@unlink(js_data_url('news',$array['news_id'],$array['news_cid'],$array['news_name'],1));
		}
		unset($where);				
		//删除新闻ID
		$where['news_id'] = $id;
