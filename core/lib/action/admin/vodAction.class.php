...
class vodAction extends baseAction{
    ...
    public function show(){
		...
		foreach($list as $key=>$val){
			...
			$list[$key]['vod_url'] = js_data_url('vod',$list[$key]['vod_id'],$list[$key]['vod_cid'],$list[$key]['vod_name'],1,$list[$key]['vod_jumpurl']);
    ...
    public function _before_insert(){
		...
		if(empty($_POST["vod_keywords"]) && C('rand_tag')){
			$_POST["vod_keywords"] = js_tag_auto($_POST["vod_name"],$_POST["vod_content"]);
		}
		//播放器组与地址组
		$play = $_POST["vod_play"];
    ...
    public function delfile($id){
		...
		if(C('url_html') > 0){	
			...
			@unlink(js_play_url_dir($array['vod_id'],0,1,$array['vod_cid'],$array['vod_name']).'js');
		}
		unset($where);
		//删除影片ID
		$where['vod_id'] = $id;
