...
class picAction extends baseAction{
    ...
    public function show(){
		$id = trim($_GET['id']);
		if ($id) {
			$dirpath = admin_js_url_repalce(str_replace('*','-',$id));
		}else{
			$dirpath = './'.C('upload_path');
		}
       // print_r($dirpath);die();
       // $newdirpath = strtolower($dirpath);
       // print_r($dirpath);die();
		...
		foreach($list_dir as $key=>$value){
			$list_dir[$key]['pathfile'] = admin_js_url_repalce($value['path'],'desc').'|'.str_replace('-','*',$value['filename']);
		}	
		if (empty($list_dir)){
	...
	public function dirlast(){
		$id = admin_js_url_repalce(trim($_GET['id']));
		if ($id) {
	...
	// 删除单个本地附件
    public function del(){
		$path = admin_js_url_repalce(str_replace('*','-',$_GET['id']));
		@unlink($path);
