...
class XmlModel extends Model {
    ...
    public function xml_insert($vod, $mustup){
		...
		// 格式化常规字符
		$vod['vod_name'] = js_xml_vodname($vod['vod_name']);
		$vod['vod_actor'] = js_xml_vodactor($vod['vod_actor']);
		$vod['vod_director'] = js_xml_vodactor($vod['vod_director']);
		// 检测来源是否完全相同
		...
		if($array){
			...
			//有相同演员时更新该影片
			$arr_actor_1 = explode(',', js_xml_vodactor($vod['vod_actor']));
			$arr_actor_2 = explode(',', js_xml_vodactor($array['vod_actor']));
		...
		$vod['vod_letter'] = js_letter_first($vod['vod_name']);
		// 随机伪原创
		if(C('play_collect')){
			$vod['vod_content'] = js_rand_str($vod['vod_content']);
		}		
		$vod['vod_stars'] = 1;
