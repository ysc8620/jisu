...
function getfirstchar($s0){
	return js_letter_first($s0);
}
function getpinyin($str,$ishead=0,$isclose=1){
	return js_pinyin($str,$ishead=0,$isclose=1);
}
...
function getfile($url){
	return js_file_get_contents($url);
}
function getvoddata($rule,$html){
	return js_preg_match($rule,$html);
}
function getvodall($rule,$html){
	return js_preg_match_all($rule,$html);
}
function getreurl($listurl){
   return js_krsort_url($listurl);
}
function getreplace($arr){
	return js_implode_rule($arr);
}
function getrole($str){
	return js_replace_rule($str);
}
function getrandstr($string){
	return  js_rand_str($string);
}
function getpicurl($file){
	return js_img_url($file);
}
function getpicurl_s($file){
	return js_img_url_small($file);
}
function getplayurl($vodid){
	return js_play_url($vodid,0,1);
}
function getactorurl($str,$type="actor"){
	return js_search_url($str,$type);
}
...
function getletter($file='vod',$str=''){
	return js_letter_url($file='vod',$str='');
}
...
// 汉字转拼单
function js_pinyin($str,$ishead=0,$isclose=1){
	$str = u2g($str);//转成GBK
	global $pinyins;
	$restr = '';
	$str = trim($str);
	...
	if(count($pinyins)==0){
		$fp = fopen('./core/common/pinyin.dat','r');
		while(!feof($fp)){
			$line = trim(fgets($fp));
...
/*-------------------------------------------------采集函数开始------------------------------------------------------------------*/
// 采集内核
function js_file_get_contents($url,$timeout=10,$referer){
...
// 采集-匹配规则结果
function js_preg_match($rule,$html){
	$arr = explode('$$$',$rule);
	if(count($arr) == 2){
...
// 采集-匹配规则结果all
function js_preg_match_all($rule,$html){
	$arr = explode('$$$',$rule);
	if(count($arr) == 2){
...
// 采集-倒序采集
function js_krsort_url($listurl){
   krsort($listurl);
...
//获取绑定分类对应ID值
function js_bind_id($key){
	$bindcache = F('_xml/bind');
	return $bindcache[$key];
}
//TAG分词自动获取
function js_tag_auto($title,$content){
	$data = js_file_get_contents('http://keyword.discuz.com/related_kw.html?ics=utf-8&ocs=utf-8&title='.rawurlencode($title).'&content='.rawurlencode(msubstr($content,0,500)));
	if($data) {
...
// 格式化采集影片名称
function js_xml_vodname($vodname){
...
// 格式化采集影片主演
function js_xml_vodactor($vodactor){
...
// 读出qvod地址的文件名
function js_qvod_name($jiname){
	$jiname = js_preg_match('3$$$(.*)\|(.*)\|(.*)\.(.*)\|',$jiname);
	return $jiname;
}
// 处理积分样式
...
// 获取与处理人气值
function js_get_hits($sidname,$type='hits',$array,$js=true){
...
// 递归多维数组转为一级数组
function js_arrays2array($array){
	static $result_array=array();
	foreach($array as $value){
		if(is_array($value)){
			js_arrays2array($value);
		}else{ 
			$result_array[]=$value;
		}
	}
...
// 返回下一篇或上一篇的内容的信息
function js_detail_array($sid='vod', $type='next', $id, $cid, $field='vod_id,vod_cid,vod_status,vod_name,vod_jumpurl'){
	//优先读取缓存数据
...
function str_replace_dir($urldir,$id,$cid,$name){
	...
	$new = array($cid,getlistname($cid,'list_dir'),js_pinyin($name),$id,md5($id),'');
...
*/
function js_list_url($sid,$arrurl,$page){
	//静态模式
	if(C('url_html') && C('url_html_list') && in_array($sid,array('vod','news'))){
		$showurl = C('site_path').str_replace('index'.C('html_file_suffix'),'',js_list_url_dir($sid,$arrurl['id'],$page).C('html_file_suffix'));
		return $showurl;
	}else{
...
* $page 分页数字,大于1时返回的URL带有分页跳转参数变量{!page!}
*/
function js_list_url_dir($sid,$cid,$page){
	//影视或文章
	if('vod' == $sid){
...
* $jumpurl 跳转地址
*/
function js_data_url($sid,$id,$cid,$name,$page,$jumpurl){
	//有跳转地址
	if ($jumpurl) {
		return $jumpurl;
	}
	//静态
	if(C('url_html')){
		$readurl = C('site_path').str_replace('index'.C('html_file_suffix'),'',js_data_url_dir($sid,$id,$cid,$name,$page).C('html_file_suffix'));
		return $readurl;
	}
	//动态模式
	$arrurl['id'] = $id;
...
* $page 分页数字,大于1时返回的URL带有分页跳转参数变量{!page!}
*/
function js_data_url_dir($sid,$id,$cid,$name,$page){
	//专题直接返回
	if('special' == $sid){
...
function js_special_url($page){
	if(C('url_html')){
		return C('site_path').str_replace('index'.C('html_file_suffix'),'',js_special_url_dir($page).C('html_file_suffix'));
	}else{
		return UU('home-special/show','',false,true);
	}
}
//获取专题静态模式目录结构URL
function js_special_url_dir($page){
...
// 获取某图片的访问地址
function js_img_url($file,$content,$number=1){
	if(!$file){
		return js_img_url_preg($file,$content,$number);
	}
...
// 获取某图片的缩略图地址
function js_img_url_small($file,$content,$number=1){
	if(!$file){
		return js_img_url_preg($file,$content,$number);
	}
	if(strpos($file,'http://') !== false){
...
//正则提取正文里指定的第几张图片地址
function js_img_url_preg($file,$content,$number=1){
...
// 内容页Tag链接
function js_content_url($content,$array_tag='',$sid=''){
	if($array_tag){
		foreach($array_tag as $key=>$value){
			$content = str_replace($value['tag_name'],'<a href="'.js_tag_url($value['tag_name'],$sid).'">'.$value['tag_name'].'</a>',$content);
		}
	}
	return $content;
}
// 自定义模板链接
function js_mytpl_url($tplname){
...
// 获取26个字母链接
function js_letter_url($file='vod',$str=''){
	if(C('url_html')){
		$index='index.html';
	}else{
		$index='index.php';
...
function js_param_url(){
	...
	$where['order'] = js_order_by($_GET['order']);
	return $where;
}
//分页跳转参数处理
function js_param_jump($where){
	if($where['sid']){
...
//返回安全的orderby
function js_order_by($order = 'addtime'){
	if(empty($order)){
		return 'addtime';
	}
	$array = array();
...
/******************************************
* @处理影视标签函数
* @以字符串方式传入,通过js_param_lable函数解析为以下变量
...
function js_mysql_news($tag){
	...
	foreach($list as $key=>$val){
		...
		$list[$key]['news_readurl'] = js_data_url('news',$list[$key]['news_id'],$list[$key]['news_cid'],$list[$key]['news_name'],1,$list[$key]['news_jumpurl']);
		$list[$key]['news_picurl'] = js_img_url($list[$key]['news_pic'],$list[$key]['news_content']);
		$list[$key]['news_picurl_small'] = js_img_url_small($list[$key]['news_pic'],$list[$key]['news_content']);
	}
	//是否写入数据缓存
	if(C('data_cache_newsforeach') && C('currentpage') < 2 ){
...
function js_mysql_special($tag){
	...
	//循环赋值
	foreach($list as $key=>$val){
		$list[$key]['special_readurl'] = js_data_url('special',$list[$key]['special_id'],0,$list[$key]['special_name'],1,'');
		$list[$key]['special_logo'] = js_img_url($list[$key]['special_logo'],$list[$key]['special_content']);
		$list[$key]['special_banner'] = js_img_url_small($list[$key]['special_banner'],$list[$key]['special_content']);
	}
	//是否写入数据缓存
	if(C('data_cache_specialforeach') && C('currentpage') < 2 ){
 ...
 * @param $data
 */
function js_play_redirect($data){
   // print_r($data);
    foreach($data['vod_playlist'] as $paylist){
        if($paylist['playname'] == $data['vod_playname']){
            foreach($paylist['son'] as $pay){
                if($pay['playpath'] == $data['vod_playpath']){
                    print_r($pay['playpath']);
                    break;
                }
            }
            break;
